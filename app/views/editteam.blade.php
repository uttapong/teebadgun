		...
		$("#editteam").submit(function(e)
		{
			...
			$.ajax(
			{
				...
				success:function(data, textStatus, jqXHR) 
				{
					...
					console.log(data);
					if(objtype=='string')bootbox.alert("คุณแก้ไขข้อมูลทีมเรียบร้อยแล้ว",function() {
...
<div class="col-md-8 left">
	...
	<h1 class="title">แก้ไขข้อมูลทีม</h1>
	<div class="row">
		
		<div class="col-md-12 left">
			<div id="open-team">

				{{ Form::model($team, array('route' => array('updateteam', $team->id),'enctype'=>'multipart/form-data','id'=>'editteam','role'=>'form','class'=>'form-horizontal','method'=>'post')) }}
				...
				<div class="form-group">
					...
					@if($team->logo)
					<div class='col-sm-6'>
						<img width="100" height="100" src="{{ asset('uploads/teams/') }}/{{$team->id}}/{{$team->logo}}" />
					</div>
					@endif
				...
				<div class="form-group">
					...
					<div class="col-sm-10" id="managerlist">
						...
						<p><span class="label label-info"><button onclick="remove_hidden_manager('{{ $manager->getValue() }}')" type="button" class="close-manager" data-dismiss="alert" aria-hidden="true">&times;</button>{{ $manager->getName() }}</span></p>
						@endforeach
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<div id="courtlist">
							...
							<div class="alert alert-info alert-dismissable"><button onclick="remove_hidden_court('gym{{ $openday->id }}')" type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h4>{{ $openday->getGymName() }}</h4><p> วัน <span class="label label-primary">{{ $openday->getDay() }}</span></p><p> เวลา {{ $openday->start_time }}น. ถึง {{ $openday->end_time }}น.</p></div>
							@endforeach	
				...
				@foreach($managers as $manager)
				{{ Form::hidden('manager[]',$manager->getValue()) }}
				@endforeach

				@foreach($opendays as $openday)
				{{ Form::hidden('court[]',$openday->getValue(),array('id'=> 'gym'.$openday->id)) }}
				@endforeach
				{{ Form::close() }}
			</div>
		</div>
